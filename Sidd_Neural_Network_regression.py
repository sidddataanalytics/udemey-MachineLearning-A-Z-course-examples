...
dataset = pd.read_csv(r'C:\Sid Data\BITS\4th Sem\Udemey\Part 8 - Deep Learning\Section 39 - Artificial Neural Networks (ANN)\Artificial_Neural_Networks\Churn_Modelling.csv')
...
#Label Encoder - to code the code the categorical data
from sklearn.preprocessing import LabelEncoder, OneHotEncoder
x_encoder_country = LabelEncoder()
x_data[:,1] = x_encoder_country.fit_transform(x_data[:,1])
print (x_data)

x_encoder_sex = LabelEncoder()
x_data[:,2] = x_encoder_sex.fit_transform(x_data[:,2])
print (x_data)

#dummy encoding using Onehtencoder to transfortm them into rows of values
x_hot_encoder = OneHotEncoder(categorical_features =[1])
...
x_test = sc_x.fit_transform(pd.DataFrame(x_test))
...
pre_proessing = pre_proessing.fit(x_data[:,3:13])
x_data[:,1:3] =  pre_proessing.transform(x_data[:,3:13)
print (x_data)
...
plt.contourf(X1, X2, naiveb.predict(np.array([X1.ravel(), X2.ravel()]).T).reshape(X1.shape),
